...
public static String parseMarkup(@NotNull Guild guild, @NotNull String message) {
  ...
  while (matcher.find()) {
    escapedMessage = escapedMessage.replace(
            matcher.group(), "<h1 class=\"markup__header\">%s</h1>".formatted(matcher.group(1)));
  }
  
  matcher = HEADER_2.matcher(escapedMessage);
  while (matcher.find()) {
    escapedMessage = escapedMessage.replace(
            matcher.group(), "<h2 class=\"markup__header\">%s</h2>".formatted(matcher.group(1)));
  }
  
  matcher = HEADER_3.matcher(escapedMessage);
  while (matcher.find()) {
    escapedMessage = escapedMessage.replace(
            matcher.group(), "<h3 class=\"markup__header\">%s</h3>".formatted(matcher.group(1)));
  }
  ...
  while (matcher.find()) {
    ...
    if (user == null) {
      escapedMessage = escapedMessage.replace(matcher.group(), "<span class=\"mention\">@%s</span>".formatted(userId));
    ...
    escapedMessage = escapedMessage.replace(
            matcher.group(),
            "<a href=\"https://discord.com/users/%s\" class=\"mention\">@%s</a>".formatted(userId, user.getEffectiveName()));
  }
  ...
  while (matcher.find()) {
    ...
    escapedMessage = escapedMessage.replace(
            matcher.group(),
            "<span class=\"mention\" style=\"color: #%1$s; background-color: #%1$s10;\">@%2$s</span>"
                    .formatted(Integer.toHexString(role.getColorRaw()), role.getName()));
  }
